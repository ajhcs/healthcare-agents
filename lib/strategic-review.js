...
function evaluateStrategicReview(request) {
  ...
  const body = {
    ...
    claim_candidates: request.frozen_inputs.claim_candidates,
...
function validateStrategicReview(review) {
  ...
  rejectUnknownKeys(review, ['schema_version', 'review_id', 'request_id', 'review_tier', 'protocol', 'reviewer', 'input_hashes', 'claim_candidates', 'evidence_bundle_ref', 'identity_binding_ref', 'decision_scenario_ref', 'first_assessment_hash', 'exposure_status', 'evidence_mutated', 'posture_assessments', 'claim_dispositions', 'missing_evidence_requests', 'method_challenges', 'prohibited_claims', 'preserved_reviewer_concerns', 'overall_disposition', 'professional_disposition_authority', 'evaluation', 'output_sha256'], 'review', messages);
  ...
  const claimIds = new Set();
  const evidenceRefsByClaim = new Map();
  for (const claim of review.claim_candidates || []) {
  ...
  if (protocol) {
    const evidenceRefs = new Set((review.claim_candidates || []).flatMap(item => item.evidence_refs || []));
