...
function evaluateStrategicReview(request) {
  ...
  const body = {
    ...
    review_id: deriveReviewId(request.request_id, request.reviewer.reviewer_id, protocol.protocol_hash, firstAssessmentHash),
...
function validateStrategicReview(review) {
  ...
  rejectUnknownKeys(review, ['schema_version', 'review_id', 'request_id', 'review_tier', 'protocol', 'reviewer', 'input_hashes', 'frozen_inputs', 'frozen_inputs_hash', 'evidence_bundle_ref', 'identity_binding_ref', 'decision_scenario_ref', 'first_assessment_hash', 'exposure_status', 'evidence_mutated', 'posture_assessments', 'claim_dispositions', 'missing_evidence_requests', 'method_challenges', 'prohibited_claims', 'preserved_reviewer_concerns', 'overall_disposition', 'professional_disposition_authority', 'evaluation', 'output_sha256'], 'review', messages);
  ...
  for (const field of ['frozen_inputs_hash', 'first_assessment_hash', 'output_sha256']) requireHash(review[field], field, messages);
  ...
  const reviewIdentityFields = [review.request_id, (review.reviewer || {}).reviewer_id, protocolRef.protocol_hash, review.first_assessment_hash];
...
function deriveReviewId(requestId, reviewerId, protocolHash, firstAssessmentHash) {
  return 'review:' + sha256({ request_id: requestId, reviewer_id: reviewerId, protocol_hash: protocolHash, first_assessment_hash: firstAssessmentHash }).slice(7, 39);
}
