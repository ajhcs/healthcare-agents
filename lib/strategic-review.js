...
function validateReviewRequest(request) {
  ...
  messages.push(...validateReviewRequestShape(request));
  ...
  if (protocol) validateCriterionResults(candidate.criterion_results, protocol, allowedEvidenceRefs, messages);
...
function validateStrategicReview(review) {
  ...
  messages.push(...validateStrategicReviewShape(review));
  rejectUnknownKeys(review, ['schema_version', 'review_id', 'request_id', 'review_tier', 'protocol', 'reviewer', 'input_hashes', 'evidence_bundle_ref', 'identity_binding_ref', 'decision_scenario_ref', 'first_assessment_hash', 'exposure_status', 'evidence_mutated', 'posture_assessments', 'claim_dispositions', 'missing_evidence_requests', 'method_challenges', 'prohibited_claims', 'preserved_reviewer_concerns', 'overall_disposition', 'professional_disposition_authority', 'evaluation', 'output_sha256'], 'review', messages);
  for (const field of ['review_id', 'request_id', 'evidence_bundle_ref', 'identity_binding_ref', 'decision_scenario_ref']) requireText(review[field], field, messages);
  for (const field of ['first_assessment_hash', 'output_sha256']) requireHash(review[field], field, messages);
  ...
  if (!Array.isArray(review.claim_dispositions) || !review.claim_dispositions.length) messages.push('review must preserve claim dispositions');
  ...
  if (protocol) {
    const evidenceRefs = new Set((review.claim_dispositions || []).flatMap(item => item.evidence_refs || []));
    validateCriterionResults(evaluation.criteria_results, protocol, evidenceRefs, messages);
  }
  const body = { ...review };
...
function validateCriterionResults(results, protocol, allowedEvidenceRefs, messages) {
  ...
  for (const item of results) {
    ...
    else if (!Array.isArray(item.evidence_refs)) messages.push('criterion evidence_refs must be an array');
