...
function validateStrategicReview(review) {
  ...
  if (shapeMessages.length) return shapeMessages;
  rejectUnknownKeys(review, ['schema_version', 'review_id', 'request_id', 'review_tier', 'protocol', 'reviewer', 'review_request_hash', 'input_hashes', 'frozen_inputs', 'frozen_inputs_hash', 'evidence_bundle_ref', 'identity_binding_ref', 'decision_scenario', 'decision_scenario_ref', 'evidence_boundary', 'first_assessment_hash', 'exposure_status', 'evidence_mutated', 'posture_assessments', 'claim_dispositions', 'missing_evidence_requests', 'method_challenges', 'prohibited_claims', 'preserved_reviewer_concerns', 'overall_disposition', 'professional_disposition_authority', 'evaluation', 'output_sha256'], 'review', messages);
  ...
  for (const field of ['review_request_hash', 'frozen_inputs_hash', 'first_assessment_hash', 'output_sha256']) requireHash(review[field], field, messages);
