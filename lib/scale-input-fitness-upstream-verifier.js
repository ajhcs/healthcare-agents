...
function verifyPinnedDataEvidence({ repo, manifest, upstreamDir, evidencePaths, messages, canonicalizeBundle = value => value, rebuildCount = 2 }) {
  if (rebuildCount !== 2) {
    messages.push('pinned Data verification requires exactly two isolated rebuilds');
    return;
  }
  ...
  for (let buildIndex = 0; buildIndex < rebuildCount; buildIndex += 1) {
    ...
    try {
      ...
      if (!Buffer.isBuffer(rebuiltBytes)) {
        messages.push('bundle canonicalizer must return a Buffer');
        continue;
      }
      ...
      validateBundleSemanticHash(rebuiltBytes, manifest.evidence_lineage.bundle_semantic_hash, messages);
  ...
  if (rebuiltOutputs.length !== 2) messages.push('two isolated pinned Data rebuilds did not both complete');
  else if (rebuiltOutputs.some(value => !value.equals(rebuiltOutputs[0]))) messages.push('isolated pinned Data rebuilds are not byte-identical');
}

function validateBundleSemanticHash(bytes, expectedHash, messages) {
  try {
    const bundle = JSON.parse(bytes.toString('utf8'));
    const claimedHash = bundle.bundle_sha256;
    delete bundle.bundle_sha256;
    if (claimedHash !== expectedHash || sha256(bundle) !== expectedHash) messages.push('pinned Data rebuild bundle semantic hash drift');
  } catch (error) {
    messages.push(`pinned Data rebuild bundle is not valid JSON: ${error.message}`);
  }
}
