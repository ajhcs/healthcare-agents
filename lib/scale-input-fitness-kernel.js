...
function validateKernelConfig(config) {
  ...
  const allowedKeys = new Set([
    'activeFamily', 'blockedCellMessage', 'boundaryRequiredTerms', 'canonicalRequestHashes',
    ...
    'expectedEvidenceIdentifiers', 'expectedManifestCounts', 'expectedReviewHashes', 'familyLabel',
    'handoffFamilyBlockedField', 'handoffFamilyConflictCountField', 'handoffFamilyConflictRefsField',
  ...
  for (const field of ['activeFamily', 'familyLabel', 'concernLineageLabel', 'conflictRefToken', 'downstreamBead', 'handoffFamilyBlockedField', 'handoffFamilyConflictCountField']) requireConfigText(config[field], field, errors);
  ...
  requireCountObject(config.expected, 'expected', expectedNumberFields, expectedLabelFields, errors);
  if (isPlainObject(config.expected)) {
    if (config.expected.totalCells !== config.expected.populatedCells + config.expected.blockedCells + config.expected.notResearchedCells) errors.push('expected cell-state counts must sum to totalCells');
    if (config.expected.familyCells > config.expected.blockedCells) errors.push('expected familyCells cannot exceed blockedCells');
  ...
  if (canonicalText(config.prohibitedUses) !== canonicalText(REQUIRED_PROHIBITED_USES)) errors.push('prohibitedUses must equal the closed Scale no-go set');
  ...
  requireConfigText(config.blockedCellMessage, 'blockedCellMessage', errors);
...
function requireCountObject(value, label, requiredFields, labelFields, errors) {
  if (!isPlainObject(value) || !Object.keys(value).length) {
  ...
  const allowed = new Set([...numericFields, ...labelFields]);
