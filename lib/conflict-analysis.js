...
function validateConflictRequest(request) {
  const messages = [];
  const shapeMessages = validateConflictRequestShape(request);
  ...
  const contextHashes = request.reviews.map(review => review.review_context_hash);
  if (new Set(contextHashes).size !== 1) messages.push('conflict analysis requires identical frozen review context');
  const claimSets = request.reviews.map(review => canonicalJson((review.frozen_inputs || {}).claim_candidates || []));
...
function validateConflictAnalysis(analysis) {
  ...
  const shapeMessages = validateConflictAnalysisShape(analysis);
  if (shapeMessages.length) return shapeMessages;
...
function compareReviewCollections(first, second, discrepancies) {
  ...
  compareField('criterion_evidence_or_warrant', 'evaluation.criteria_results', first, second, first.evaluation.criteria_results, second.evaluation.criteria_results, discrepancies);
}
