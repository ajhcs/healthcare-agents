...
function validatePhysicianCountUpstream(manifest, objects, artifactHashes = {}, evidenceArtifacts = {}) {
  ...
  if (manifest.active_family !== 'physician_count') messages.push('active family must remain physician_count');
  ...
  if (artifactHashes.toolkit_handoff !== TOOLKIT_HANDOFF_FILE_HASH) messages.push('Toolkit normalized handoff exact bytes must match the frozen raw hash');
  ...
  if (rawSha256(stableCanonicalJson(runtimeHandoff)) !== TOOLKIT_RUNTIME_HANDOFF_FILE_HASH) messages.push('normalized-to-runtime Toolkit handoff deterministic substitution hash drift');
  ...
  if (stateCounts.populated !== 0 || stateCounts.blocked_source_conflict !== 30 || stateCounts.not_yet_researched !== 24) messages.push('cumulative packet must preserve exactly 0 populated, 30 blocked_source_conflict, and 24 not_yet_researched cells');
  if (physicianCells.length !== 6) messages.push('cumulative packet must preserve six physician-count cells');
  for (const cell of physicianCells) {
    if (cell.state !== 'blocked_source_conflict' || cell.approved_value !== null || cell.source_backed !== false || cell.source_backed_zero !== false || cell.imputed !== false) messages.push(`physician-count cell must remain blocked and unapproved: ${cell.product_system_slug}`);
    if (!Array.isArray(cell.conflict_refs) || cell.conflict_refs.length !== 1) messages.push(`physician-count cell must preserve one open conflict: ${cell.product_system_slug}`);
  }
  const physicianConflicts = (packet.unresolved_conflict_refs || []).filter(ref => ref.includes(':physician-count:'));
  if (physicianConflicts.length !== 6) messages.push('packet must preserve six physician-count conflicts');
  if ((packet.unresolved_conflict_refs || []).length !== 23) messages.push('packet must preserve twenty-three cumulative open conflicts');
...
function validatePhysicianCountReviewRequest(request, manifest, objects, artifactHashes, evidenceArtifacts) {
  const messages = validatePhysicianCountUpstream(manifest, objects, artifactHashes, evidenceArtifacts);
  const canonical = tryDeriveCanonical(objects, artifactHashes, messages);
  if (canonical) {
  ...
  if (!textIncludesAll(candidate, ['six physician-count', '23 cumulative', '26 roster/bed material discrepancies', '24 roster/bed reviewer concerns', 'ten evidence-specific overturn gates', '56 revenue-review concerns', 'two annual-review discrepancies', '172 annual-review concerns'])) messages.push('review must preserve cumulative blocker counts');
  if (canonicalText(candidate.preserved_reviewer_concerns) !== canonicalText(expectedPhysicianConcerns(objects))) messages.push('review must preserve the exact ordered prior and physician-slice concern lineage');
  ...
  if (!textIncludesAll(candidate, ['no averaging', 'no automatic adjudication', 'human'])) messages.push('review must preserve disagreement and human-authority boundaries');
  return [...new Set(messages)];
}
...
function validatePhysicianCountReviewHandoff(handoff, reviews, conflict, manifest, objects, artifactHashes, evidenceArtifacts) {
  ...
  const expectedReviews = canonical ? [
    ...
    evaluateStrategicReview(canonical.workforceRequest)
  ] : null;
  ...
  if (handoff.active_family !== 'physician_count') messages.push('handoff active_family must remain physician_count');
  const body = { ...handoff };
  ...
  if (handoff.downstream_bead !== 'healthcare-toolkit-2rr9.6.3.6') messages.push('handoff must route to the physician-count Toolkit admission bead');
  ...
  if (workforce?.protocol?.protocol_id !== 'cso.operations-access-capacity.v1' || workforce?.reviewer?.agent_slug !== 'operations-workforce-manager') messages.push('second specialist artifact must be the exact physician-workforce review role');
  ...
  const physicianConflictRefs = (objects?.cumulative_packet?.unresolved_conflict_refs || []).filter(ref => ref.includes(':physician-count:'));
  if (handoff.cumulative_open_conflict_count !== 23 || handoff.physician_count_blocked_cell_count !== 6 || handoff.physician_count_open_conflict_count !== 6) messages.push('handoff must preserve all cumulative conflicts and six blocked physician-count cells');
  if (canonicalText(handoff.physician_count_open_conflict_refs) !== canonicalText(physicianConflictRefs)) messages.push('handoff must preserve the exact six physician-count conflict refs');
  const expectedReviewHashes = { methods: reviews?.[0]?.output_sha256, physician_workforce: reviews?.[1]?.output_sha256 };
  const expectedAssessmentHashes = { methods: reviews?.[0]?.first_assessment_hash, physician_workforce: reviews?.[1]?.first_assessment_hash };
...
function deriveCanonical(objects, artifactHashes) {
  return derivePhysicianCountCanonical({ objects, artifactHashes });
}
...
function tryDeriveCanonical(objects, artifactHashes, messages) {
  try {
    ...
    messages.push(`canonical physician-count derivation failed closed: ${error.message}`);
    return null;
  }
}
