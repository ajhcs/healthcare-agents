...
const { deriveAnnualDischargesCanonical } = require('./scale-annual-discharges-canonical');
...
function deriveCanonical(objects, artifactHashes) {
  ...
  for (const [role, artifactRef] of Object.entries(OBJECT_ARTIFACT_REFS)) {
    objectEntries[role] = { artifact_ref: artifactRef, artifact_hash: artifactHashes?.[role] };
    const semantic = semanticHash(objects?.[role]);
    if (semantic) objectEntries[role].semantic_hash = semantic;
  }
  return deriveAnnualDischargesCanonical({
    objects,
    objectEntries,
    evidencePaths: {
