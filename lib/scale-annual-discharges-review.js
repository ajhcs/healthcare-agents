...
const TOOLKIT_FEATURE = '7dccc91afca9efb26d59c8d6b01534cbe4db4809';
const TOOLKIT_TRACKER = '420d35d8024de1c484c1b16128836e0f8b00375c';
const DATA_PRODUCER = 'cbefd15c9e15ce84d8b00fe0d54f67d946cb6ccd';
const DATA_FEATURE = 'ebe381c357c80c68640bf74841867dc4cbff930b';
const DATA_TRACKER = '9b7ffb55710ecb0a3920710f5589dde8fa53b04f';
const TOOLKIT_HANDOFF_FILE_HASH = 'sha256:2c217a84f520a5443c7d5561e8ea9312bbd13ec4f5d541c226287a4c465fb0fe';
const EVIDENCE_BUNDLE_REF = 'ushso-rebuild://scale-inputs/annual-discharges/public-evidence-bundle';
const COMMITTED_INPUT_REF = `git:${DATA_PRODUCER}:contracts/v2/fixtures/scale-annual-discharges-input.json`;
const ACQUISITION_REF = `git:${DATA_PRODUCER}:contracts/v2/fixtures/scale-annual-discharges-acquisition.json`;
const ACQUISITION_RAW_HASH = 'sha256:aa0027e2af3dc5e29fc2e5245b6e3d36370b83560ed8bbf64f9de12c6908495a';
const ACQUISITION_SEMANTIC_HASH = 'sha256:fc5b401a8f63a98b0bbdc601e47948336e6200ad1177bbac7ded34df387ebcae';
const NORMALIZED_INPUT_RAW_HASH = 'sha256:29229692c230073770d5ecbd766d385bd2b9f44eb5c6be2d8640d5480b0fc1d3';
const PRODUCER_BOUND_INPUT_RAW_HASH = 'sha256:d12085e3987df0765f94cffa82576f9f3b452042affb57638ac2b66c4462eb5f';
const EVIDENCE_BUNDLE_RAW_HASH = 'sha256:72f3c121b55b505b4779cbe3ee362c25aa8a4b73202849cfa4ae5ca97bbe0da4';
const EVIDENCE_BUNDLE_SEMANTIC_HASH = 'sha256:b8cad4dc793c86c1015dd0929cbb42d125158fc258d5569dccc8371b7dbd00a9';
...
function validateAnnualDischargesReviewRequest(request, manifest, objects, artifactHashes, evidenceArtifacts) {
  ...
  if (!Array.isArray(candidate.preserved_reviewer_concerns) || candidate.preserved_reviewer_concerns.length < 86) messages.push('review must preserve all prior and slice reviewer concerns');
...
function validateAnnualDischargesReviewHandoff(handoff, reviews, conflict, manifest, objects, artifactHashes, evidenceArtifacts) {
  ...
  if (handoff.cumulative_open_conflict_count !== 17 || handoff.annual_discharges_blocked_cell_count !== 6) messages.push('handoff must preserve all cumulative conflicts and six blocked annual-discharges cells');
  if (handoff.review_hashes?.methods !== reviews?.[0]?.output_sha256 || handoff.review_hashes?.utilization_operations !== reviews?.[1]?.output_sha256) messages.push('handoff review hashes must match exact specialist outputs');
  if (handoff.first_assessment_hashes?.methods !== reviews?.[0]?.first_assessment_hash || handoff.first_assessment_hashes?.utilization_operations !== reviews?.[1]?.first_assessment_hash) messages.push('handoff first-assessment hashes must match exact specialist assessments');
