...
function validateEvidencePackRegistry(registry = loadEvidencePackRegistry(), workflows = []) {
  ...
  for (const pack of registry.packs) {
    ...
    if (pack.status === 'active' && Array.isArray(pack.citation_cards) && pack.workflow_id !== 'denial-spike-workup' && pack.citation_cards.length < 8) {
      messages.push(pack.id + ': active standard packs must include at least 8 citation cards');
    }
    if (workflow && pack.workflow_id !== 'denial-spike-workup') {
      const requiredFieldText = (Array.isArray(pack.citation_cards) ? pack.citation_cards : [])
        .flatMap(card => Array.isArray(card.required_fields) ? card.required_fields : [])
        .map(normalizeTitle);
      const requiredFieldSet = new Set(requiredFieldText);
      for (const input of workflow.required_inputs || []) {
        if (!requiredFieldSet.has(normalizeTitle(input))) {
          messages.push(pack.id + ': required workflow input is not covered by citation card required_fields: ' + input);
        }
      }
    }
