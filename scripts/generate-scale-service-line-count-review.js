...
const OUT = path.join(ROOT, 'review-protocols', 'fixtures', 'scale-input-packets', 'service-line-count');
const UPSTREAM = path.join(OUT, 'upstream');
const roles = Object.fromEntries(Object.entries(SERVICE_LINE_COUNT_CANONICAL_CONTEXT.objectArtifactRefs)
  .map(([role, artifactRef]) => [role, artifactRef.replace(/^upstream\//, '')]));
const evidencePaths = SERVICE_LINE_COUNT_CANONICAL_CONTEXT.evidencePaths;
...
const handoffBody = {
  ...
  review_hashes: { methods: methodsReview.output_sha256, service_portfolio_governance: governanceReview.output_sha256 },
  first_assessment_hashes: { methods: methodsReview.first_assessment_hash, service_portfolio_governance: governanceReview.first_assessment_hash },
  conflict_output_hash: conflictAnalysis.output_sha256,
  prior_counts: PRIOR_COUNTS,
  service_line_count_unavailable_cell_count: 6,
  service_line_count_open_conflict_count: 6,
  service_line_count_open_conflict_refs: serviceLineConflicts,
  cumulative_open_conflict_count: 29,
  cumulative_cell_counts: { total: 54, populated: 0, blocked_source_conflict: 30, unavailable_public: 6, not_yet_researched: 18 },
  ...
  downstream_bead: 'healthcare-toolkit-2rr9.6.3.8'
};
const handoff = { ...handoffBody, handoff_sha256: sha256(handoffBody) };
const handoffMessages = validateServiceLineCountReviewHandoff(handoff, [methodsReview, governanceReview], conflictAnalysis, upstreamManifest, objects, artifactHashes, evidenceArtifacts);
...
const outputs = {
  ...
  'service_portfolio_governance-review-request.json': governanceRequest,
  'methods-review.json': methodsReview,
  'service_portfolio_governance-review.json': governanceReview,
...
console.log(`Generated service-line-count fitness review: ${methodsReview.output_sha256}, ${governanceReview.output_sha256}, ${conflictAnalysis.output_sha256}, ${handoff.handoff_sha256}`);
