...
const OUT = path.join(ROOT, 'review-protocols', 'fixtures', 'scale-input-packets', 'safety-net-patient-mix');
const UPSTREAM = path.join(OUT, 'upstream');
const roles = Object.fromEntries(Object.entries(SAFETY_NET_PATIENT_MIX_CANONICAL_CONTEXT.objectArtifactRefs)
  .map(([role, artifactRef]) => [role, artifactRef.replace(/^upstream\//, '')]));
const evidencePaths = SAFETY_NET_PATIENT_MIX_CANONICAL_CONTEXT.evidencePaths;
...
const {
  upstreamManifest, methodsRequest, populationRequest, priorDiscrepancies,
  safetyNetConflicts
} = deriveSafetyNetPatientMixCanonical({
  objects,
  artifactHashes
...
const handoffBody = {
  ...
  review_hashes: { methods: methodsReview.output_sha256, population_health: populationReview.output_sha256 },
  first_assessment_hashes: { methods: methodsReview.first_assessment_hash, population_health: populationReview.first_assessment_hash },
  ...
  safety_net_patient_mix_pct_unavailable_cell_count: 6,
  safety_net_patient_mix_pct_open_conflict_count: 6,
  safety_net_patient_mix_pct_open_conflict_refs: safetyNetConflicts,
  cumulative_open_conflict_count: 35,
  cumulative_cell_counts: { total: 54, populated: 0, blocked_source_conflict: 30, unavailable_public: 12, not_yet_researched: 12 },
  ...
  downstream_bead: 'healthcare-toolkit-2rr9.6.3.10'
};
const handoff = { ...handoffBody, handoff_sha256: sha256(handoffBody) };
const handoffMessages = validateSafetyNetPatientMixReviewHandoff(handoff, [methodsReview, populationReview], conflictAnalysis, upstreamManifest, objects, artifactHashes, evidenceArtifacts);
...
const outputs = {
  ...
  'population_health-review-request.json': populationRequest,
  'methods-review.json': methodsReview,
  'population_health-review.json': populationReview,
...
console.log(`Generated safety-net-patient-mix fitness review: ${methodsReview.output_sha256}, ${populationReview.output_sha256}, ${conflictAnalysis.output_sha256}, ${handoff.handoff_sha256}`);
