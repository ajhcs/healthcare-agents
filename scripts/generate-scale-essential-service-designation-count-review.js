...
const OUT = path.join(ROOT, 'review-protocols', 'fixtures', 'scale-input-packets', 'essential-service-designation-count');
const UPSTREAM = path.join(OUT, 'upstream');
const roles = Object.fromEntries(Object.entries(ESSENTIAL_SERVICE_DESIGNATION_COUNT_CANONICAL_CONTEXT.objectArtifactRefs)
  .map(([role, artifactRef]) => [role, artifactRef.replace(/^upstream\//, '')]));
const evidencePaths = ESSENTIAL_SERVICE_DESIGNATION_COUNT_CANONICAL_CONTEXT.evidencePaths;
...
const {
  upstreamManifest, methodsRequest, regulatoryRequest, priorDiscrepancies,
  essentialServiceDesignationConflicts
} = deriveEssentialServiceDesignationCountCanonical({
  objects,
  artifactHashes
});
const methodsReview = evaluateStrategicReview(methodsRequest);
const regulatoryReview = evaluateStrategicReview(regulatoryRequest);
const conflictRequest = {
  schema_version: 'ushso.ai-conflict-analysis-request.v1',
  request_id: 'conflict-request:scale-essential-service-designation-count:2026-07-19',
  review_tier: 'ordinary_material_claim',
  reviews: [methodsReview, regulatoryReview]
};
const conflictAnalysis = analyzeReviewConflicts(conflictRequest);
const handoffBody = {
  schema_version: 'ushso.scale-input-fitness-review-handoff.v1',
  active_family: 'essential_service_designation_count',
  ...
  review_hashes: { methods: methodsReview.output_sha256, regulatory: regulatoryReview.output_sha256 },
  first_assessment_hashes: { methods: methodsReview.first_assessment_hash, regulatory: regulatoryReview.first_assessment_hash },
  ...
  essential_service_designation_count_unavailable_cell_count: 6,
  essential_service_designation_count_open_conflict_count: 6,
  essential_service_designation_count_open_conflict_refs: essentialServiceDesignationConflicts,
  cumulative_open_conflict_count: 47,
  cumulative_cell_counts: { total: 54, populated: 0, blocked_source_conflict: 30, unavailable_public: 24, not_yet_researched: 0 },
  ...
  downstream_bead: 'healthcare-toolkit-2rr9.6.3.14'
};
const handoff = { ...handoffBody, handoff_sha256: sha256(handoffBody) };
const handoffMessages = validateEssentialServiceDesignationCountReviewHandoff(handoff, [methodsReview, regulatoryReview], conflictAnalysis, upstreamManifest, objects, artifactHashes, evidenceArtifacts);
...
const outputs = {
  ...
  'regulatory-review-request.json': regulatoryRequest,
  'methods-review.json': methodsReview,
  'regulatory-review.json': regulatoryReview,
...
console.log(`Generated essential-service-designation-count fitness review: ${methodsReview.output_sha256}, ${regulatoryReview.output_sha256}, ${conflictAnalysis.output_sha256}, ${handoff.handoff_sha256}`);
