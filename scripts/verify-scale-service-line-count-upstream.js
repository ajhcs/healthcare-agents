...
const { stablePrettyJson, validateServiceLineCountUpstream } = require('../lib/scale-service-line-count-review');
const {
  verifyAncestryEdges,
...
if (repoFlag < 0 || !args[repoFlag + 1] || toolkitFlag < 0 || !args[toolkitFlag + 1]) {
  console.error('usage: verify-scale-service-line-count-upstream.js --data-mcp-repo PATH --toolkit-repo PATH');
  process.exit(2);
}
...
const messages = validateServiceLineCountUpstream(manifest, objects, hashes, evidenceArtifacts);
verifyAncestryEdges([
...
const toolkitPaths = {
  cumulative_packet: 'contracts/reusable-run/v3/fixtures/service-line-count/cumulative-packet.json',
  decision_scenario: 'contracts/reusable-run/v3/fixtures/service-line-count/decision-scenario.json',
  identity_binding: 'contracts/reusable-run/v3/fixtures/service-line-count/identity-binding.json',
  no_execution_result: 'contracts/reusable-run/v3/fixtures/service-line-count/no-execution-result.json',
  process_claim: 'contracts/reusable-run/v3/fixtures/service-line-count/process-claim.json',
  toolkit_handoff: 'contracts/reusable-run/v3/fixtures/service-line-count/handoff.json',
  prior_cumulative_packet: 'contracts/reusable-run/v3/fixtures/physician-count/cumulative-packet.json',
  prior_review_record: 'contracts/reusable-run/v3/fixtures/physician-count-assurance/cumulative-review-record.json',
  prior_assurance_case: 'contracts/reusable-run/v3/fixtures/physician-count-assurance/cumulative-module-assurance-case.json'
};
...
console.log(`Scale service-line-count upstream verified: Toolkit ${manifest.producer_pins.healthcare_toolkit}, Data MCP ${manifest.producer_pins.healthcare_data_mcp}, handoff ${manifest.toolkit_handoff_file_hash}`);
