...
const generatedNames = ['upstream/data-mcp/producer-bound-input.json', 'upstream/data-mcp/public-evidence-bundle.json', 'upstream-manifest.json', 'methods-review-request.json', 'physician_workforce-review-request.json', 'methods-review.json', 'physician_workforce-review.json', 'conflict-analysis-request.json', 'conflict-analysis.json', 'handoff.json'];
const objectPaths = {
...
function mutateAndValidate(mutator) {
  ...
  mutator(mutated);
  return validatePhysicianCountUpstream(manifest, mutated, artifactHashes, evidenceArtifacts).join('; ');
}
...
function validateSynchronizedMutation(manifestMutator, requestMutator = () => {}) {
  ...
  const mutatedWorkforceRequest = clone(workforceRequest);
  ...
  mutatedHandoff.review_hashes = {
    ...
    physician_workforce: mutatedWorkforce.output_sha256
  };
  mutatedHandoff.first_assessment_hashes = {
    methods: mutatedMethods.first_assessment_hash,
    physician_workforce: mutatedWorkforce.first_assessment_hash
  };
  ...
  return [
    ...validatePhysicianCountUpstream(mutatedManifest, objects, artifactHashes, evidenceArtifacts),
    ...validatePhysicianCountReviewRequest(mutatedMethodsRequest, mutatedManifest, objects, artifactHashes, evidenceArtifacts),
    ...validatePhysicianCountReviewRequest(mutatedWorkforceRequest, mutatedManifest, objects, artifactHashes, evidenceArtifacts),
    ...validatePhysicianCountReviewHandoff(mutatedHandoff, [mutatedMethods, mutatedWorkforce], mutatedConflict, mutatedManifest, objects, artifactHashes, evidenceArtifacts)
  ].join('; ');
}

const before = new Map(generatedNames.map(name => [name, fs.readFileSync(path.join(FIXTURES, name))]));
for (let build = 0; build < 2; build += 1) {
  const generation = spawnSync(process.execPath, [path.join(ROOT, 'scripts', 'generate-scale-physician-count-review.js')], { cwd: ROOT, encoding: 'utf8' });
...
const workforceRequest = load('physician_workforce-review-request.json');
const methods = load('methods-review.json');
const workforce = load('physician_workforce-review.json');
...
assert.deepStrictEqual(PHYSICIAN_COUNT_CANONICAL_CONTEXT.evidencePaths, evidencePaths);
assert.deepStrictEqual(
  derivePhysicianCountCanonical({ objects, artifactHashes }).upstreamManifest,
  manifest
);
assert.deepStrictEqual(validatePhysicianCountUpstream(manifest, objects, artifactHashes, evidenceArtifacts), []);
for (const request of [methodsRequest, workforceRequest]) {
  assert.deepStrictEqual(validateReviewRequestShape(request), []);
  assert.deepStrictEqual(validateReviewRequest(request), []);
  assert.deepStrictEqual(validatePhysicianCountReviewRequest(request, manifest, objects, artifactHashes, evidenceArtifacts), []);
...
assert.deepStrictEqual(methodsRequest.frozen_inputs, workforceRequest.frozen_inputs);
assert.deepStrictEqual(methodsRequest.decision_scenario, workforceRequest.decision_scenario);
...
assert.strictEqual(workforce.protocol.protocol_id, 'cso.operations-access-capacity.v1');
assert.strictEqual(workforce.reviewer.agent_slug, 'operations-workforce-manager');
...
assert.strictEqual(methodsRequest.candidate_review.preserved_reviewer_concerns.length, 202);
assert.strictEqual(workforceRequest.candidate_review.preserved_reviewer_concerns.length, 202);
assert.strictEqual(conflict.preserved_reviewer_concerns.length, 404);
assert.deepStrictEqual(validatePhysicianCountReviewHandoff(handoff, [methods, workforce], conflict, manifest, objects, artifactHashes, evidenceArtifacts), []);
assert.strictEqual(handoff.downstream_bead, 'healthcare-toolkit-2rr9.6.3.6');
assert.deepStrictEqual(handoff.cumulative_cell_counts, { total: 54, populated: 0, blocked_source_conflict: 30, not_yet_researched: 24 });
...
assert.deepStrictEqual(
  objects.identity_binding.observation_bindings.map(binding => binding.source_candidate_value),
  [1054, 3811, 1281, 4336, 1012, 1084]
);
assert(objects.identity_binding.observation_bindings.every(binding => binding.approved_value === null && binding.input_state === 'blocked_source_conflict'));
...
for (const ref of [
  `git:${manifest.producer_pins.healthcare_data_mcp}:contracts/scale-inputs/v1/fixtures/physician-count/public-evidence-bundle.json`,
  ...
  assert.match(validatePhysicianCountReviewRequest(badRef, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /truthful deterministic rebuild URI/);
}
...
assert.match(validatePhysicianCountUpstream(wrongPath, objects, artifactHashes, evidenceArtifacts).join('; '), /committed evidence input Git path drift/);
for (const field of ['normalized_input_artifact_ref', 'producer_bound_input_artifact_ref', 'bundle_artifact_ref']) {
  ...
  assert.match(validatePhysicianCountUpstream(wrongArtifactRef, objects, artifactHashes, evidenceArtifacts).join('; '), /must resolve to the exact packaged evidence artifact/);
}
for (const [role, expected] of [
  ...
  assert.match(validatePhysicianCountUpstream(manifest, objects, artifactHashes, driftedArtifacts).join('; '), expected);
}
...
assert.match(validatePhysicianCountUpstream(manifest, objects, artifactHashes, normalizedContentDrift).join('; '), /producer-bound evidence input must differ only|deterministic rebuild hash mismatch/);
const boundContentDrift = clone(evidenceArtifacts);
boundContentDrift.producer_bound_input.value.producer.commit = '9'.repeat(40);
assert.match(validatePhysicianCountUpstream(manifest, objects, artifactHashes, boundContentDrift).join('; '), /producer-bound evidence input must differ only|commit drift/);
...
assert.match(validatePhysicianCountUpstream(manifest, objects, artifactHashes, bundleSemanticDrift).join('; '), /semantic self-hash drift/);
...
assert.match(validatePhysicianCountUpstream(manifest, objects, artifactDrift, evidenceArtifacts).join('; '), /exact artifact bytes drift|frozen raw hash/);
const commitDrift = clone(manifest);
commitDrift.producer_pins.healthcare_toolkit = '1'.repeat(40);
assert.match(validatePhysicianCountUpstream(commitDrift, objects, artifactHashes, evidenceArtifacts).join('; '), /self-hash|Toolkit producer pin drift/);
for (const [field, expected] of [
  ...
  assert.match(validatePhysicianCountUpstream(driftedProvenance, objects, artifactHashes, evidenceArtifacts).join('; '), expected);
}
...
assert.match(validatePhysicianCountUpstream(semanticRepin, objects, artifactHashes, evidenceArtifacts).join('; '), /semantic hash drift|exact Toolkit handoff pin/);
...
for (const role of Object.keys(objectPaths)) {
  for (const locator of ['upstream/does-not-exist.json', undefined]) {
    ...
    assert.match(validatePhysicianCountUpstream(locatorDrift, objects, artifactHashes, evidenceArtifacts).join('; '), /exact artifact locator and hashes/);
  }
}
...
assert.match(mutateAndValidate(value => value.prior_review_record.current_preserved_concerns.pop()), /172 annual-review concerns/);
assert.match(mutateAndValidate(value => value.prior_review_record.current_discrepancies.pop()), /two annual-review discrepancies/);
assert.match(mutateAndValidate(value => value.prior_review_record.open_conflict_refs.pop()), /seventeen open conflicts/);
assert.match(mutateAndValidate(value => value.cumulative_packet.unresolved_conflict_refs.pop()), /twenty-three cumulative open conflicts/);
assert.match(mutateAndValidate(value => { value.cumulative_packet.cells.find(cell => cell.input_family === 'physician_count').state = 'populated'; }), /blocked and unapproved/);
assert.match(mutateAndValidate(value => { value.cumulative_packet.cells.find(cell => cell.input_family === 'physician_count').source_backed_zero = true; }), /blocked and unapproved/);
assert.match(mutateAndValidate(value => { value.cumulative_packet.cells.find(cell => cell.input_family === 'physician_count').approved_value = 0; }), /blocked and unapproved/);
assert.match(mutateAndValidate(value => { value.identity_binding.observation_bindings[0].approved_value = 1054; }), /upstream manifest must exactly equal canonical derivation|semantic hash drift/);
...
assert.match(mutateAndValidate(value => { value.identity_binding.observation_bindings[0].source_basis = 'employed current deduplicated physicians'; }), /upstream manifest must exactly equal canonical derivation|semantic hash drift/);
...
assert.match(mutateAndValidate(value => { value.cumulative_packet.comparability_gates.find(gate => gate.dimension === 'physician_basis').status = 'passed'; }), /all ten comparability gates must remain unresolved|physician basis gate must remain blocked/);
assert.match(mutateAndValidate(value => { value.cumulative_packet.cells.find(cell => cell.state === 'not_yet_researched').state = 'blocked_source_conflict'; }), /exactly 0 populated, 30 blocked_source_conflict, and 24 not_yet_researched/);
assert.match(mutateAndValidate(value => { value.cumulative_packet.cells.find(cell => cell.state === 'blocked_source_conflict').state = 'populated'; }), /exactly 0 populated, 30 blocked_source_conflict, and 24 not_yet_researched/);
...
assert.match(mutateAndValidate(value => { value.cumulative_packet = null; }), /canonical physician-count derivation failed closed/);

const weakened = clone(methodsRequest);
weakened.candidate_review.overall_disposition = 'pass';
assert.match(validatePhysicianCountReviewRequest(weakened, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /must remain block/);
const missingCounts = clone(methodsRequest);
missingCounts.candidate_review.preserved_reviewer_concerns = [];
assert.match(validatePhysicianCountReviewRequest(missingCounts, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /exact ordered prior and physician-slice concern lineage/);
...
assert.match(validatePhysicianCountReviewRequest(replacedConcern, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /exact ordered prior and physician-slice concern lineage/);
...
assert.match(validatePhysicianCountReviewRequest(fabricatedEvidence, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /evidence reference absent from frozen manifest/);
...
assert.match(validatePhysicianCountReviewHandoff(averaged, [methods, workforce], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /cannot fabricate authority, adjudicate, or average/);
const authority = clone(handoff);
authority.human_authority_conveyed = true;
assert.match(validatePhysicianCountReviewHandoff(authority, [methods, workforce], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /cannot fabricate authority/);
const promoted = clone(handoff);
promoted.output_inventory.promotion_attempts = 1;
assert.match(validatePhysicianCountReviewHandoff(promoted, [methods, workforce], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /inventory zero/);
for (const prohibitedUses of [[], handoff.prohibited_uses.slice(1), [...handoff.prohibited_uses].reverse()]) {
  ...
  assert.match(validatePhysicianCountReviewHandoff(weakenedUses, [methods, workforce], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /exact ordered prohibited-use set/);
}
...
assert.match(validatePhysicianCountReviewHandoff(wrongActiveFamily, [methods, workforce], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /active_family must remain physician_count/);
...
assert.match(validatePhysicianCountReviewHandoff(extraHandoffField, [methods, workforce], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /closed review-handoff field set/);
...
assert.match(validatePhysicianCountReviewHandoff(incompleteOutputInventory, [methods, workforce], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /exact closed zero-output inventory|inventory zero/);
const lostSliceConflictCount = clone(handoff);
lostSliceConflictCount.physician_count_open_conflict_count = 0;
...
assert.match(validatePhysicianCountReviewHandoff(lostSliceConflictCount, [methods, workforce], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /six blocked physician-count cells/);
const lostSliceConflictRef = clone(handoff);
lostSliceConflictRef.physician_count_open_conflict_refs.pop();
...
assert.match(validatePhysicianCountReviewHandoff(lostSliceConflictRef, [methods, workforce], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /exact six physician-count conflict refs/);
const rehashedMethods = clone(methods);
rehashedMethods.reviewer.reviewer_id = 'scale-physician-count:methods:substituted';
rehashedMethods.output_sha256 = sha256(Object.fromEntries(Object.entries(rehashedMethods).filter(([key]) => key !== 'output_sha256')));
assert.match(validatePhysicianCountReviewHandoff(handoff, [rehashedMethods, workforce], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /mutually link the exact specialist review IDs and hashes|review hashes must match exact specialist outputs/);
...
assert.match(validatePhysicianCountReviewHandoff(handoff, [rehashedConcernReview, workforce], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /specialist reviews must exactly equal canonical evaluation/);
...
assert.match(validatePhysicianCountReviewHandoff(handoff, [methods, workforce], rehashedConflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /conflict review_refs must mutually link|conflict output hash must match/);
...
assert.match(validatePhysicianCountReviewHandoff(handoff, [methods, workforce], deletedConflictDiscrepancy, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /exactly equal deterministic recomputation/);
...
assert.match(validatePhysicianCountReviewHandoff(handoff, [methods, workforce], deletedConflictConcern, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /exactly equal deterministic recomputation/);
...
assert.match(validatePhysicianCountReviewHandoff(handoff, [methods, wrongWorkforceRole], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /exact physician-workforce review role/);
for (const [field, value, expected] of [
  ...
  assert.match(validatePhysicianCountReviewHandoff(changed, [methods, workforce], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), expected);
}
...
assert.match(validatePhysicianCountReviewHandoff(runtimeHandoffDrift, [methods, workforce], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /runtime handoff raw hash/);
...
assert.match(validatePhysicianCountReviewHandoff(firstAssessmentDrift, [methods, workforce], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /first-assessment hashes must (?:exactly )?match/);
...
assert.match(validatePhysicianCountReviewHandoff(handoffCountDrift, [methods, workforce], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /cumulative cell counts must equal/);

const workforceText = JSON.stringify(workforce).toLowerCase();
for (const term of ['total_mds', 'employed', 'affiliated', 'credential', 'active', 'deduplic', 'app-inclusive', 'current', 'workforce', 'not zero']) assert(workforceText.includes(term), `workforce review must preserve ${term}`);
console.log('Scale physician-count independent-first review, lineage, adversarial gates, and downstream handoff validated.');
