...
const FIXTURES = path.join(ROOT, 'review-protocols', 'fixtures', 'scale-input-packets', 'emergency-department-count');
const UPSTREAM = path.join(FIXTURES, 'upstream');
const generatedNames = ['upstream/data-mcp/producer-bound-input.json', 'upstream/data-mcp/public-evidence-bundle.json', 'upstream-manifest.json', 'methods-review-request.json', 'operations-review-request.json', 'methods-review.json', 'operations-review.json', 'conflict-analysis-request.json', 'conflict-analysis.json', 'handoff.json'];
const objectPaths = {
...
function mutateAndValidate(mutator) {
  ...
  mutator(mutated);
  return validateEmergencyDepartmentCountUpstream(manifest, mutated, artifactHashes, evidenceArtifacts).join('; ');
}
...
function validateSynchronizedMutation(manifestMutator, requestMutator = () => {}) {
  ...
  mutatedHandoff.review_hashes = {
    ...
    operations: mutatedOperations.output_sha256
  };
  mutatedHandoff.first_assessment_hashes = {
    methods: mutatedMethods.first_assessment_hash,
    operations: mutatedOperations.first_assessment_hash
  };
  ...
  return [
    ...validateEmergencyDepartmentCountUpstream(mutatedManifest, objects, artifactHashes, evidenceArtifacts),
    ...validateEmergencyDepartmentCountReviewRequest(mutatedMethodsRequest, mutatedManifest, objects, artifactHashes, evidenceArtifacts),
    ...validateEmergencyDepartmentCountReviewRequest(mutatedOperationsRequest, mutatedManifest, objects, artifactHashes, evidenceArtifacts),
    ...validateEmergencyDepartmentCountReviewHandoff(mutatedHandoff, [mutatedMethods, mutatedOperations], mutatedConflict, mutatedManifest, objects, artifactHashes, evidenceArtifacts)
  ].join('; ');
}

const before = new Map(generatedNames.map(name => [name, fs.readFileSync(path.join(FIXTURES, name))]));
for (let build = 0; build < 2; build += 1) {
  const generation = spawnSync(process.execPath, [path.join(ROOT, 'scripts', 'generate-scale-emergency-department-count-review.js')], { cwd: ROOT, encoding: 'utf8' });
...
assert.deepStrictEqual(EMERGENCY_DEPARTMENT_COUNT_CANONICAL_CONTEXT.evidencePaths, evidencePaths);
assert.deepStrictEqual(
  deriveEmergencyDepartmentCountCanonical({ objects, artifactHashes }).upstreamManifest,
  manifest
);
assert.deepStrictEqual(validateEmergencyDepartmentCountUpstream(manifest, objects, artifactHashes, evidenceArtifacts), []);
for (const request of [methodsRequest, operationsRequest]) {
  ...
  assert.deepStrictEqual(validateEmergencyDepartmentCountReviewRequest(request, manifest, objects, artifactHashes, evidenceArtifacts), []);
...
assert.strictEqual(operations.reviewer.independence.prior_exposure, 'none');
...
const operationsRequestText = JSON.stringify(operationsRequest);
for (const leaked of [operationsRequest.request_id, operationsRequest.reviewer.reviewer_id, operations.review_id, operations.output_sha256]) assert(!methodsRequestText.includes(leaked), `methods request must not leak later operations review state: ${leaked}`);
for (const leaked of [methodsRequest.request_id, methodsRequest.reviewer.reviewer_id, methods.review_id, methods.output_sha256]) assert(!operationsRequestText.includes(leaked), `operations request must not leak methods review state: ${leaked}`);
...
assert.strictEqual(methodsRequest.candidate_review.preserved_reviewer_concerns.length, 244);
assert.strictEqual(operationsRequest.candidate_review.preserved_reviewer_concerns.length, 246);
assert.deepStrictEqual(methodsRequest.candidate_review.preserved_reviewer_concerns.slice(0, 230), objects.prior_review_record.current_methods_preserved_concerns);
assert.deepStrictEqual(operationsRequest.candidate_review.preserved_reviewer_concerns.slice(0, 232), objects.prior_review_record.current_population_health_preserved_concerns);
assert.strictEqual(conflict.preserved_reviewer_concerns.length, 490);
assert.deepStrictEqual(validateEmergencyDepartmentCountReviewHandoff(handoff, [methods, operations], conflict, manifest, objects, artifactHashes, evidenceArtifacts), []);
assert.strictEqual(handoff.downstream_bead, 'healthcare-toolkit-2rr9.6.3.12');
assert.deepStrictEqual(handoff.cumulative_cell_counts, { total: 54, populated: 0, blocked_source_conflict: 30, unavailable_public: 18, not_yet_researched: 6 });
...
assert.strictEqual(handoff.prior_discrepancy_ids.length, 38);
assert.strictEqual(handoff.current_discrepancy_ids.length, 3);
assert.strictEqual(handoff.cumulative_discrepancy_count, 41);
...
for (const ref of [
  `git:${manifest.producer_pins.healthcare_data_mcp}:contracts/scale-inputs/v1/fixtures/emergency-department-count/public-evidence-bundle.json`,
  ...
  assert.match(validateEmergencyDepartmentCountReviewRequest(badRef, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /truthful deterministic rebuild URI/);
}
...
assert.match(validateEmergencyDepartmentCountUpstream(wrongPath, objects, artifactHashes, evidenceArtifacts).join('; '), /committed evidence input Git path drift/);
for (const field of ['normalized_input_artifact_ref', 'producer_bound_input_artifact_ref', 'bundle_artifact_ref']) {
  ...
  assert.match(validateEmergencyDepartmentCountUpstream(wrongArtifactRef, objects, artifactHashes, evidenceArtifacts).join('; '), /must resolve to the exact packaged evidence artifact/);
}
for (const [role, expected] of [
  ...
  assert.match(validateEmergencyDepartmentCountUpstream(manifest, objects, artifactHashes, driftedArtifacts).join('; '), expected);
}
...
assert.match(validateEmergencyDepartmentCountUpstream(manifest, objects, artifactHashes, normalizedContentDrift).join('; '), /producer-bound evidence input must differ only|deterministic rebuild hash mismatch/);
const boundContentDrift = clone(evidenceArtifacts);
boundContentDrift.producer_bound_input.value.producer.commit = '9'.repeat(40);
assert.match(validateEmergencyDepartmentCountUpstream(manifest, objects, artifactHashes, boundContentDrift).join('; '), /producer-bound evidence input must differ only|commit drift/);
...
assert.match(validateEmergencyDepartmentCountUpstream(manifest, objects, artifactHashes, bundleSemanticDrift).join('; '), /semantic self-hash drift/);
...
assert.match(validateEmergencyDepartmentCountUpstream(manifest, objects, artifactDrift, evidenceArtifacts).join('; '), /exact artifact bytes drift|frozen raw hash/);
const commitDrift = clone(manifest);
commitDrift.producer_pins.healthcare_toolkit = '1'.repeat(40);
assert.match(validateEmergencyDepartmentCountUpstream(commitDrift, objects, artifactHashes, evidenceArtifacts).join('; '), /self-hash|Toolkit producer pin drift/);
for (const [field, expected] of [
  ...
  assert.match(validateEmergencyDepartmentCountUpstream(driftedProvenance, objects, artifactHashes, evidenceArtifacts).join('; '), expected);
}
...
assert.match(validateEmergencyDepartmentCountUpstream(semanticRepin, objects, artifactHashes, evidenceArtifacts).join('; '), /semantic hash drift|exact Toolkit handoff pin/);
...
for (const role of Object.keys(objectPaths)) {
  for (const locator of ['upstream/does-not-exist.json', undefined]) {
    ...
    assert.match(validateEmergencyDepartmentCountUpstream(locatorDrift, objects, artifactHashes, evidenceArtifacts).join('; '), /exact artifact locator and hashes/);
  }
}
...
assert.match(mutateAndValidate(value => value.prior_review_record.prior_service_line_discrepancies.pop()), /three service-line discrepancies|38 prior review discrepancies/);
assert.match(mutateAndValidate(value => value.prior_review_record.current_discrepancies.pop()), /three safety-net discrepancies|38 prior review discrepancies/);
assert.match(mutateAndValidate(value => value.prior_review_record.current_methods_preserved_concerns.pop()), /230-item methods ancestry/);
assert.match(mutateAndValidate(value => value.prior_review_record.current_population_health_preserved_concerns.pop()), /232-item population-health ancestry/);
assert.match(mutateAndValidate(value => value.prior_review_record.open_conflict_refs.pop()), /thirty-five open conflicts/);
assert.match(mutateAndValidate(value => value.cumulative_packet.unresolved_conflict_refs.pop()), /forty-one cumulative open conflicts/);
assert.match(mutateAndValidate(value => { value.cumulative_packet.cells.find(cell => cell.input_family === 'emergency_department_count').state = 'populated'; }), /unavailable, unapproved, and unpopulated/);
assert.match(mutateAndValidate(value => { value.cumulative_packet.cells.find(cell => cell.input_family === 'emergency_department_count').source_backed_zero = true; }), /unavailable, unapproved, and unpopulated/);
assert.match(mutateAndValidate(value => { value.cumulative_packet.cells.find(cell => cell.input_family === 'emergency_department_count').approved_value = 0; }), /unavailable, unapproved, and unpopulated/);
...
assert.match(mutateAndValidate(value => { value.identity_binding.observation_bindings[0].source_basis = 'summed facility Emergency Services flags'; }), /upstream manifest must exactly equal canonical derivation|semantic hash drift/);
...
assert.match(mutateAndValidate(value => { value.cumulative_packet.comparability_gates.find(gate => gate.dimension === 'emergency_department_definition').status = 'passed'; }), /all ten comparability gates must remain unresolved|emergency-department definition gate must remain blocked|nine blocked/);
assert.match(mutateAndValidate(value => { value.cumulative_packet.cells.find(cell => cell.state === 'not_yet_researched').state = 'blocked_source_conflict'; }), /exactly 0 populated, 30 blocked_source_conflict, 18 unavailable_public, and 6 not_yet_researched/);
assert.match(mutateAndValidate(value => { value.cumulative_packet.cells.find(cell => cell.state === 'unavailable_public').state = 'populated'; }), /exactly 0 populated, 30 blocked_source_conflict, 18 unavailable_public, and 6 not_yet_researched/);
...
assert.match(mutateAndValidate(value => { value.cumulative_packet = null; }), /canonical emergency-department-count derivation failed closed/);

const weakened = clone(methodsRequest);
weakened.candidate_review.overall_disposition = 'pass';
assert.match(validateEmergencyDepartmentCountReviewRequest(weakened, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /must remain block/);
const missingCounts = clone(methodsRequest);
missingCounts.candidate_review.preserved_reviewer_concerns = [];
assert.match(validateEmergencyDepartmentCountReviewRequest(missingCounts, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /exact ordered 230\/232-item prior lane ancestry/);
...
assert.match(validateEmergencyDepartmentCountReviewRequest(replacedConcern, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /exact ordered 230\/232-item prior lane ancestry/);
const reorderedConcern = clone(operationsRequest);
...
assert.match(validateEmergencyDepartmentCountReviewRequest(reorderedConcern, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /exact ordered 230\/232-item prior lane ancestry/);
...
assert.match(validateEmergencyDepartmentCountReviewRequest(fabricatedEvidence, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /evidence reference absent from frozen manifest/);
...
assert.match(validateEmergencyDepartmentCountReviewHandoff(averaged, [methods, operations], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /cannot fabricate authority, adjudicate, or average/);
const authority = clone(handoff);
authority.human_authority_conveyed = true;
assert.match(validateEmergencyDepartmentCountReviewHandoff(authority, [methods, operations], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /cannot fabricate authority/);
const promoted = clone(handoff);
promoted.output_inventory.promotion_attempts = 1;
assert.match(validateEmergencyDepartmentCountReviewHandoff(promoted, [methods, operations], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /inventory zero/);
for (const prohibitedUses of [[], handoff.prohibited_uses.slice(1), [...handoff.prohibited_uses].reverse()]) {
  ...
  assert.match(validateEmergencyDepartmentCountReviewHandoff(weakenedUses, [methods, operations], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /exact ordered prohibited-use set/);
}
...
assert.match(validateEmergencyDepartmentCountReviewHandoff(wrongActiveFamily, [methods, operations], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /active_family must remain emergency_department_count/);
...
assert.match(validateEmergencyDepartmentCountReviewHandoff(extraHandoffField, [methods, operations], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /closed review-handoff field set/);
...
assert.match(validateEmergencyDepartmentCountReviewHandoff(incompleteOutputInventory, [methods, operations], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /exact closed zero-output inventory|inventory zero/);
const lostSliceConflictCount = clone(handoff);
lostSliceConflictCount.emergency_department_count_open_conflict_count = 0;
...
assert.match(validateEmergencyDepartmentCountReviewHandoff(lostSliceConflictCount, [methods, operations], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /six unavailable_public emergency-department-count cells/);
const lostUnavailableCount = clone(handoff);
lostUnavailableCount.emergency_department_count_unavailable_cell_count = 0;
...
assert.match(validateEmergencyDepartmentCountReviewHandoff(lostUnavailableCount, [methods, operations], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /six unavailable_public emergency-department-count cells/);
const lostSliceConflictRef = clone(handoff);
lostSliceConflictRef.emergency_department_count_open_conflict_refs.pop();
...
assert.match(validateEmergencyDepartmentCountReviewHandoff(lostSliceConflictRef, [methods, operations], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /exact six emergency-department-count conflict refs/);
...
assert.match(validateEmergencyDepartmentCountReviewHandoff(lostPriorDiscrepancy, [methods, operations], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /all 38 prior discrepancy IDs/);
...
assert.match(validateEmergencyDepartmentCountReviewHandoff(reorderedPriorDiscrepancy, [methods, operations], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /all 38 prior discrepancy IDs/);
const rehashedMethods = clone(methods);
rehashedMethods.reviewer.reviewer_id = 'scale-emergency-department-count:methods:substituted';
rehashedMethods.output_sha256 = sha256(Object.fromEntries(Object.entries(rehashedMethods).filter(([key]) => key !== 'output_sha256')));
assert.match(validateEmergencyDepartmentCountReviewHandoff(handoff, [rehashedMethods, operations], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /mutually link the exact specialist review IDs and hashes|review hashes must match exact specialist outputs/);
...
assert.match(validateEmergencyDepartmentCountReviewHandoff(handoff, [rehashedConcernReview, operations], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /specialist reviews must exactly equal canonical evaluation/);
...
assert.match(validateEmergencyDepartmentCountReviewHandoff(handoff, [methods, operations], rehashedConflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /conflict review_refs must mutually link|conflict output hash must match/);
...
assert.match(validateEmergencyDepartmentCountReviewHandoff(handoff, [methods, operations], deletedConflictDiscrepancy, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /exactly equal deterministic recomputation/);
...
assert.match(validateEmergencyDepartmentCountReviewHandoff(handoff, [methods, operations], deletedConflictConcern, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /exactly equal deterministic recomputation/);
...
assert.match(validateEmergencyDepartmentCountReviewHandoff(handoff, [methods, wrongOperationsRole], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /exact emergency-access operations\/access\/capacity review role/);
for (const [field, value, expected] of [
  ...
  assert.match(validateEmergencyDepartmentCountReviewHandoff(changed, [methods, operations], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), expected);
}
...
assert.match(validateEmergencyDepartmentCountReviewHandoff(runtimeHandoffDrift, [methods, operations], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /runtime handoff raw hash/);
...
assert.match(validateEmergencyDepartmentCountReviewHandoff(firstAssessmentDrift, [methods, operations], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /first-assessment hashes must (?:exactly )?match/);
...
assert.match(validateEmergencyDepartmentCountReviewHandoff(handoffCountDrift, [methods, operations], conflict, manifest, objects, artifactHashes, evidenceArtifacts).join('; '), /cumulative cell counts must equal/);

const operationsText = JSON.stringify(operations).toLowerCase();
for (const term of ['ahrq', 'emergency services', '42 cfr 489.24', 'dedicated emergency', 'campus', 'installed capacity', 'staffed capacity', 'throughput', 'unavailable', 'not zero']) assert(operationsText.includes(term), `operations review must preserve ${term}`);
console.log('Scale emergency-department-count independent-first review, lineage, adversarial gates, and downstream handoff validated.');
