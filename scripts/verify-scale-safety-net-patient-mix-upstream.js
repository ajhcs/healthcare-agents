...
const { stablePrettyJson, validateSafetyNetPatientMixUpstream } = require('../lib/scale-safety-net-patient-mix-review');
const {
  verifyAncestryEdges,
...
if (repoFlag < 0 || !args[repoFlag + 1] || toolkitFlag < 0 || !args[toolkitFlag + 1]) {
  console.error('usage: verify-scale-safety-net-patient-mix-upstream.js --data-mcp-repo PATH --toolkit-repo PATH');
  process.exit(2);
}
...
const messages = validateSafetyNetPatientMixUpstream(manifest, objects, hashes, evidenceArtifacts);
verifyAncestryEdges([
...
const toolkitPaths = {
  cumulative_packet: 'contracts/reusable-run/v3/fixtures/safety-net-patient-mix/cumulative-packet.json',
  decision_scenario: 'contracts/reusable-run/v3/fixtures/safety-net-patient-mix/decision-scenario.json',
  identity_binding: 'contracts/reusable-run/v3/fixtures/safety-net-patient-mix/identity-binding.json',
  no_execution_result: 'contracts/reusable-run/v3/fixtures/safety-net-patient-mix/no-execution-result.json',
  process_claim: 'contracts/reusable-run/v3/fixtures/safety-net-patient-mix/process-claim.json',
  toolkit_handoff: 'contracts/reusable-run/v3/fixtures/safety-net-patient-mix/handoff.json',
  prior_cumulative_packet: 'contracts/reusable-run/v3/fixtures/service-line-count/cumulative-packet.json',
  prior_review_record: 'contracts/reusable-run/v3/fixtures/service-line-count-assurance/cumulative-review-record.json',
  prior_assurance_case: 'contracts/reusable-run/v3/fixtures/service-line-count-assurance/cumulative-module-assurance-case.json'
};
...
console.log(`Scale safety-net-patient-mix upstream verified: Toolkit ${manifest.producer_pins.healthcare_toolkit}, Data MCP ${manifest.producer_pins.healthcare_data_mcp}, handoff ${manifest.toolkit_handoff_file_hash}`);
