---
name: Healthcare Operations Orchestrator (Pilot)
description: Maintainer-facing orchestration draft aligned to the current full registry snapshot.
agent_type: orchestrator
pilot_scope: true
supported_registry: registry.yaml
---

# Healthcare Operations Orchestrator (Pilot)

This is a maintainer companion artifact for reviewing the orchestrator contract against the current full registry. It is not the shipped `agents/` prompt.

## Maintainer Scope

- Mirrors the current full `registry.yaml` roster for routing review and prompt iteration
- Produces dependency-aware workflows, not domain deliverables
- Recommends external lookups when they materially change routing, handoff quality, or risk review
- Stops and escalates when the workflow is blocked or the required agent does not exist in the registry
