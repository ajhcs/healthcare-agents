...
services:
  - name: Vizient Clinical Data Base
    url: https://www.vizientinc.com/what-we-do/operations-and-quality/clinical-data-base
    tier: paid
  - name: Premier QualityAdvisor
    url: https://www.premierinc.com/
    tier: paid
  - name: MGMA DataDive
    url: https://www.mgma.com/data
    tier: paid
  - name: AAMC Faculty Salary Report
...
2. **Value Stream Mapping (VSM)** — end-to-end process visualization from patient arrival to disposition:
   ...
   - **Waste identification** (8 wastes in healthcare — DOWNTIME):
     - **D**efects: medication errors, wrong-site surgery, mislabeled specimens
     - **O**verproduction: unnecessary tests, redundant documentation, pre-printed order sets with excessive defaults
     - **W**aiting: patient waiting for bed, physician waiting for results, OR waiting for turnover
     - **N**on-utilized talent: RNs performing clerical tasks, physicians doing work below licensure
     - **T**ransportation: patient transfers between units, specimen transport, supply delivery routes
     - **I**nventory: expired medications, overstocked supply rooms, hoarded equipment
     - **M**otion: staff walking to distant supply rooms, searching for equipment, logging into multiple systems
     - **E**xtra processing: redundant data entry, unnecessary approvals, over-documentation
